...
trait Configuration {
  def videoName = "video4.mp4" // the video to parse
  ...
  def replayDetection(filename: String, knownLogoTag: Option[String] = None) = {
    ...
    logos
      ...
      .foreach{case (optTag, logos) =>
        OpenCvUtils.saveFrames(capture, logos.map(_.index), frameFolderName + optTag.getOrElse("unk") + "/", Some(runId + "/"), mosaicSize)
      }
