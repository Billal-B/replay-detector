import org.bytedeco.javacpp.indexer.{FloatIndexer, IntIndexer}
  ...
  private def computeRankTracing(histograms: Seq[(Mat, Mat, Mat)]): Int = {
    ...
    histograms.foreach {
      case ((hHist, sHist, vHist)) =>
        val hists = new Mat()
        hconcat(new MatVector(hHist, sHist, vHist), hists) // todo : use java list instead of asJava
        xt.push_back(hists.reshape(0, 1))
    }
    SVDecomp(xt, w, u, vt)

    val bestSvalue: Float = w.createIndexer[FloatIndexer]().get(0,0,0)
    val res = (0 until windowSize).foldLeft(0) { // todo : replace windowSize by histograms.length
      case (tot, idx) =>
        val sValue:Float = w.createIndexer[FloatIndexer]().get(idx,0,0)
