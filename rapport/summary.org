...
Pour apprendre ces caractéristiques, l'entrée du réseau ne va pas être une image (2D), mais une séquence d'image (3D).
C'est sur ce point que cette approche diffère de Two-Stream Convolutional Networks for Action Recognition in Videos.
Pour prédire la classe d'un frame à l'instant t, il faut faire une fusion de la sortie du réseau spatial et du réseau temporel.
De plus, l'apprentissage du réseau temporel est séparé de celui du réseau réseau spatial, ce qui ne permet pas au réseau de capturer l'information temporel relativement à l'information spatial.
Même si le réseau temporel prend plusieurs frames en entrée (paramètre L > 1), la convolution 2D va écraser la composante temporelle.
