...
** Détection des plans
Les approches que nous proposons itèrent sur tous les frames de la vidéo, à la recherche des logos pouvant se trouver au début et à la fin des replays.
...
- un histogramme HSV
Néanmoins, aucune de ces méthodes ne permet de vectoriser l'image de telle sorte qu'un algorithme comme K-Means obtienne de bons résultats, ces caractéristiques ne sont pas assez discriminantes.
...
Ici, nous ne récupérons les caractéristiques que d'une seule image par plan. L'image pour laquelle nous extrayons les caractéristiques est la dernière frame de la séquence, car c'est la dernière image faisant la transition entre deux plans (et dans le cas des logos, c'est dans celle-ci que sa forme visuelle est la plus nette).
...
#+CAPTION: Caractéristiques extraites par ORB (en vert) pour un plan (agrégat des caractéristiques de chaque image dans le plan) label:orb-window
#+ATTR_LATEX: :width 8cm
...
- Dans certaines vidéos, les logos de début et fin de replay ne sont pas les mêmes, cette approche ne permet pas de détecter les replays dans ce cas
- Dans certains vidéos, il y a des logos au début des replays, mais pas de logo à la fin des replays (un simple fondu remplace le logo).

Puisque cette méthode est rapide et ne renvoie presque pas de faux positifs, elle nous servira par la suite pour créer un ensemble de données d'image avec des logos de replay.
...
-  les données d'entraînement ne sont pas assez représentatives (II) : la majorité des séquences logos (~90%) que nous avons dans l'ensemble d'entraînement sont des logos de Liga. Ceci est du au fait que la plupart des vidéos de match de football entier auquel nous avons pu accéder sont des vidéos de Liga.
